...
class BertSelfAttention(nn.Module):
  ...
  def attention(self, key, query, value, attention_mask):
    ...
    d_k = query.size(-1)  # key.shape = query.shape = value.shape = (batch_size, n_heads, seq_len, attension_head_size)
    scores = query @ key.transpose(2, 3)/math.sqrt(d_k) # S[*, i, j, k]表示第 i 个头里token i对 token k的注意力分数
    ...
    output = output.reshape(bs, seq_len, self.all_head_size)
...
class BertLayer(nn.Module):
  ...
  def forward(self, hidden_states, attention_mask):
    ...
    intermediate_output = self.interm_af(intermediate_output) # Gelu

    new_hidden_states = self.add_norm(hidden_states, intermediate_output, self.out_dense, self.out_dropout, self.out_layer_norm)
